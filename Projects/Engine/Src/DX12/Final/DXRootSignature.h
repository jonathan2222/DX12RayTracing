...
namespace RS::DX12
{
    ...
    class DXDescriptorCache;

    class DXRootParameter
    {
        friend class DXRootSignature;
    public:
    ...
    {
        friend class DXDynamicDescriptorHeap;

    public:

        DXRootSignature(UINT NumRootParams = 0, UINT NumStaticSamplers = 0) : m_Finalized(FALSE), m_NumParameters(NumRootParams)
        {
        ...
        {
            if (NumRootParams > 0)
                m_ParamArray.reset(new DXRootParameter[NumRootParams]);
            else
        ...
        DXRootParameter& operator[] (size_t EntryIndex)
        {
            RS_ASSERT(EntryIndex < m_NumParameters);
            return m_ParamArray.get()[EntryIndex];
        }

        const DXRootParameter& operator[] (size_t EntryIndex) const
        {
            RS_ASSERT(EntryIndex < m_NumParameters);
            return m_ParamArray.get()[EntryIndex];
        }
    ...
    protected:
        ...
        std::unique_ptr<DXRootParameter[]> m_ParamArray;
