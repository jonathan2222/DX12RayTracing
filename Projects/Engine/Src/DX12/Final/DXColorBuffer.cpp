...
void RS::DX12::DXColorBuffer::CreateFromSwapChain(const std::wstring& Name, ID3D12Resource* BaseResource)
{
    AssociateWithResource(DXCore::GetDevice(), Name, BaseResource, D3D12_RESOURCE_STATE_PRESENT);
    ...
    m_RTVHandle = DXCore::AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
    DXCore::GetDevice()->CreateRenderTargetView(m_pResource.Get(), nullptr, m_RTVHandle);
}

void RS::DX12::DXColorBuffer::Create(const std::wstring& Name, uint32_t Width, uint32_t Height, uint32_t NumMips, DXGI_FORMAT Format, D3D12_GPU_VIRTUAL_ADDRESS VidMemPtr)
{
    ...
    ClearValue.Color[0] = m_ClearColor.Red;
    ClearValue.Color[1] = m_ClearColor.Green;
    ClearValue.Color[2] = m_ClearColor.Blue;
    ClearValue.Color[3] = m_ClearColor.Alpha;

    CreateTextureResource(DXCore::GetDevice(), Name, ResourceDesc, ClearValue, VidMemPtr);
    CreateDerivedViews(DXCore::GetDevice(), Format, 1, NumMips);
}

void RS::DX12::DXColorBuffer::CreateArray(const std::wstring& Name, uint32_t Width, uint32_t Height, uint32_t ArrayCount, DXGI_FORMAT Format, D3D12_GPU_VIRTUAL_ADDRESS VidMemPtr)
{
    D3D12_RESOURCE_FLAGS Flags = CombineResourceFlags();
    ...
    CreateTextureResource(DXCore::GetDevice(), Name, ResourceDesc, ClearValue, VidMemPtr);
    CreateDerivedViews(DXCore::GetDevice(), Format, ArrayCount, 1);
}

void RS::DX12::DXColorBuffer::GenerateMipMaps(DXCommandContext& BaseContext)
{
    if (m_NumMipMaps == 0)
        return;

    DXComputeContext& Context = BaseContext.GetComputeContext();

    Context.SetRootSignature(RenderCore::GetCommonRootSignature());
    ...
    for (uint32_t TopMip = 0; TopMip < m_NumMipMaps; )
    {
        ...
        if (m_Format == DXGI_FORMAT_R8G8B8A8_UNORM_SRGB)
            Context.SetPipelineState(RenderCore::GenerateMipsGammaPSO[NonPowerOfTwo]);
        else
            Context.SetPipelineState(RenderCore::GenerateMipsLinearPSO[NonPowerOfTwo]);
...
void RS::DX12::DXColorBuffer::CreateDerivedViews(ID3D12Device* Device, DXGI_FORMAT Format, uint32_t ArraySize, uint32_t NumMips)
{
    RS_ASSERT(ArraySize == 1 || NumMips == 1, "We don't support auto-mips on texture arrays");
    ...
    {
        m_RTVHandle = DXCore::AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
        m_SRVHandle = DXCore::AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    }
    ...
    for (uint32_t i = 0; i < NumMips; ++i)
    {
        if (m_UAVHandle[i].ptr == D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN)
            m_UAVHandle[i] = DXCore::AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
